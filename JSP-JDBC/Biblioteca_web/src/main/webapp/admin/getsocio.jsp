...
<body>
	...
	<div id="formBusquedaSocio" class="formulariogeneral">
		<form name="frmBusquedaSocio" id="frmBusquedaSocio" method="post" action="${pageContext.request.contextPath}/controllerAdmin">
			...
			<legend><img src="${pageContext.request.contextPath}/resources/img/azarquiel.gif">&nbsp;Introduzca parte del nombre del Socio a modificar</legend>
					...
					<div class="campos">			
                           <input type="search"
                                  name="frmbusquedanombre" 
						   id="frmbusquedanombre" 
						   value="${iniciales}" size="25" maxlength="40">
 ...
 <c:if test="${listadoSociosBusqueda!=null}" >
	 ...
	 <c:choose>
		 		 ...
		 		 <tbody>
					<c:forEach items="${listadoSociosBusqueda}" var="socio">
						<tr>		           
			              ...
			              <td class="txtcentrado"><a href="${pageContext.request.contextPath}/controllerAdmin?operacion=editarsocio&socio=${socio.idsocio}">Editar</a></td>
			            </tr>		
					</c:forEach>
				</tbody>
				</table>
		...
		<c:otherwise>	
		   <p align="center" 
		      class="advertencia">
		      <c:out value="Ningún registro coincide con el patrón: ${iniciales}"></c:out>
		  </p> 
		</c:otherwise>
			</c:choose>
  </c:if>		
